...
const QuoteSummary: React.FC<QuoteSummaryProps> = ({ vehicle, result, showDetailedBreakdown = false }) => {
  ...
  const hasTaxes = (result.includeTaxes && result.taxCost && result.taxCost > 0) || 
  ...
  return (
    <Card className="p-4">
      ...
      <div className="space-y-2">
        ...
        {/* Seção de Impostos e Taxas - Modificada para exibir mesmo que aparente estar zerada */}
          ...
          <AccordionItem value="taxes" className="border-b-0">
            ...
            <AccordionContent className="pt-2">
              <div className="text-sm space-y-2 text-muted-foreground bg-slate-50 p-3 rounded-md">
                {/* Exibir IPVA mesmo que zerado */}
                {result.includeIpva && (
                  <div className="flex justify-between">
                    <span>IPVA ({(result.ipvaCost / (vehicle.value / 12) * 100).toFixed(2)}%):</span>
                    <span>{formatCurrency(result.ipvaCost)}/mês</span>
                  </div>
                )}
                
                {/* Exibir Licenciamento mesmo que zerado */}
                ...
                {/* Exibir Custos financeiros mesmo que zerado */}
                {result.includeTaxes && (
                  <>
                    <div className="flex justify-between">
