...
export const addVehicleToQuote = async (quoteId: string, vehicleData: any): Promise<{ success: boolean; data?: any; error?: any }> => {
  try {
    // @ts-ignore - Precisamos usar fetch diretamente pois a tabela quote_vehicles pode não existir no tipo
    const { data, error } = await supabase
      .from('vehicles_quotes')
      .insert({
        quote_id: quoteId,
...
export const getQuoteVehicles = async (quoteId: string): Promise<{ success: boolean; vehicles?: any[]; error?: any }> => {
  try {
    // @ts-ignore - Usando fetch diretamente para a tabela que pode não existir no tipo
    const { data, error } = await supabase
      .from('vehicles_quotes')
      .select(`
        vehicle_id,
