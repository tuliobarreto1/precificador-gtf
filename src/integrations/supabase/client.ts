  ...
  try {
    ...
    if (quote.vehicles && quote.vehicles.length > 0) {
      ...
      for (const vehicleItem of quote.vehicles) {
        ...
        // Determinar valor mensal do veículo
        let monthlyValue = 0;
        ...
        // Caso contrário, verificar se há um resultado calculado
        else if (vehicleItem.result && vehicleItem.result.totalCost !== undefined) {
          monthlyValue = vehicleItem.result.totalCost;
          console.log("Usando valor mensal do resultado calculado:", monthlyValue);
        }
        // Como último recurso, dividir o custo total pelo número de veículos
        else if (quote.totalCost && quote.vehicles.length > 0) {
          monthlyValue = quote.totalCost / quote.vehicles.length;
          console.log("Usando valor mensal calculado a partir do custo total:", monthlyValue);
        }
