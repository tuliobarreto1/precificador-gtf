      ...
      for (let i = 0; i < quote.vehicles.length; i++) {
        ...
        if (vehicle.id) {
          ...
          if (existingVehicleById && Array.isArray(existingVehicleById) && existingVehicleById.length > 0) {
            ...
            const updateData = {
              ...
              is_used: vehicle.isUsed === true || vehicle.is_used === true, // Garantir que seja boolean
        ...
        if (!vehicleId) {
          ...
          const vehicleData = {
            ...
            value: parseFloat(vehicle.value) || 0,
            plate_number: vehicle.plateNumber || vehicle.plate_number || null,
            is_used: (vehicle.plateNumber || vehicle.plate_number) ? true : (vehicle.isUsed === true || vehicle.is_used === true), // Se tem placa, é usado
            group_id: vehicle.groupId || vehicle.group_id || 'A',
            color: vehicle.color || null,
            odometer: parseInt(vehicle.odometer) || 0,
            fuel_type: vehicle.fuelType || vehicle.fuel_type || vehicle.tipoCombustivel || null
