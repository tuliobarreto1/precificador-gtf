...
export async function saveQuoteToSupabase(quote: any) {
  try {
    ...
    if (quote.vehicles && quote.vehicles.length > 0) {
      await saveQuoteVehicles(quote, quoteId, isValidUuid);
    } else {
...
// Função auxiliar para salvar veículos do orçamento
async function saveQuoteVehicles(quote: any, quoteId: string, isUpdating: boolean) {
  try {
    ...
    for (let i = 0; i < quote.vehicles.length; i++) {
      ...
      // Se o ID não é um UUID (como 'new-123'), precisamos verificar ou criar o veículo
        ...
        if (vehicle.plateNumber || vehicle.plate_number) {
          const existingVehicle = await findVehicleByPlate(vehicle.plateNumber || vehicle.plate_number);
          if (existingVehicle) {
        ...
        if (!vehicleId && vehicle.brand && vehicle.model) {
          const existingVehicle = await findVehicleByBrandModel(vehicle.brand, vehicle.model);
          if (existingVehicle) {
            vehicleId = existingVehicle.id;
        ...
        if (!vehicleId) {
          const result = await createOrUpdateVehicle(vehicle);
          if (result.success) {
            vehicleId = result.id;
            console.log("Novo veículo criado com ID:", vehicleId);
          } else {
            console.error("Erro ao criar veículo:", result.error);
            continue;
          }
        }
      }
      
      // Neste ponto, temos o ID do veículo independentemente de ser novo ou usado
      if (!vehicleId) {
    ...
    console.error("Erro ao salvar veículos do orçamento:", error);
    return { success: false, error };
  }
}
